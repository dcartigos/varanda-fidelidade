...
module.exports = async function handler(req, res) {
      ...
      //
      // ⚠️ O .trim() NÃO É ENFEITE — CORRIGIDO EM 21/08/2026
      //
      // A comparação era `tokenRecebido !== tokenEsperado`, crua. Isso gerou 401
      // durante três dias, incluindo em chamadas do PRÓPRIO SERVIDOR para si mesmo,
      // onde o valor sai e entra da mesma variável de ambiente e em teoria nunca
      // poderia divergir.
      //
      // Causa: um espaço ou quebra de linha invisível no fim do valor salvo na
      // variável do Vercel. O cabeçalho HTTP normaliza espaços em branco nas pontas;
      // a variável de ambiente não. Os dois lados deixam de bater e a mensagem de
      // erro diz apenas "Token inválido", que aponta para o lugar errado — a gente
      // procurou o valor errado, o navegador errado e o deploy errado por três dias.
      //
      // Comparar com trim resolve para sempre, seja qual for o valor guardado, e
      // ninguém precisa colar o segredo com precisão cirúrgica. Não remover.
      const limpar = (v) => String(v == null ? '' : v).trim();

      const tokenEsperado = limpar(process.env.APP_TOKEN);
      const tokenRecebido = limpar(req.headers['x-varanda-token']);
      ...
      if (tokenRecebido !== tokenEsperado) {
              // Sem vazar o valor: só o suficiente para saber SE é problema de conteúdo
        // ou de tamanho. Foi a falta disso que fez o diagnóstico levar três dias.
        return responder(res, 401, {
                  erro: 'Token inválido.',
                  diagnostico: {
                              recebeu_token: tokenRecebido.length > 0,
                              tamanho_recebido: tokenRecebido.length,
                              tamanho_esperado: tokenEsperado.length,
                              tamanhos_batem: tokenRecebido.length === tokenEsperado.length,
                  },
                  dica: 'Se os tamanhos batem mas o token é inválido, o conteúdo é outro. ' +
                              'Se não batem, foi valor errado ou truncado.',
        });
      }
